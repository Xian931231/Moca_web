const deviceList = (function () {
	...
	let _sessionParam = util.getSessionParam("external", "device");
			...
			if(list) {
				for(let item of list) {
					...
					body_o.append(tr_o);
					{
						// 상품 이름
						let td_o = $("<td>");
						let a_o = $("<a>").attr("href", "/external/device/add?product_id=" + item.product_id)
					...
					{
						// 기기 등록일
						let td_o = $("<td>").text(item.device_insert_date ? item.device_insert_date : "-");
						tr_o.append(td_o);
					}
				}
			}
