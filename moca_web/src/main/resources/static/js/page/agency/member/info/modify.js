const agencyMember = (function(){
	...
	function _evInit(){
		let evo = $("[data-src='agnecyMember'][data-act]").off();
	...
	function _action(ev){
		...
		if(type_v == "click") {
			...
			//회원 탈퇴
				...
				event.removeMember();
			//pw 초기화
			}else if(act_v == "pwInit"){
				$("#modalSearch").val("");
			//회원탈퇴 클릭
			}else if(act_v == "processingSgChk"){
				event.processingSgChk();
			}
	...
	let _event = {
		...
		removeMember: function(){
			...
			modifyMember.removeMember($("#modalSearch").val(), function(resp){
				if(resp.result){
					...
					let msg = "회원탈퇴에 실패했습니다.</br>문제가 지속될 경우 02-3214-3142로 연락 주시기 바랍니다.";
					
					if(resp.code == 2301){
						msg = "회원탈퇴에 실패했습니다.</br>문제가 지속될 경우 02-3214-3142로 연락 주시기 바랍니다.";
					}else if(resp.code == 2212){
		...
		//종료되지 않은 상품이 있는지 확인 
		processingSgChk: function(){
			$("#modalSearch").val("");
			
			url_v = "/member/agency/processing/Sg";
			comm.send(url_v, null, "POST", function(resp){
				if(resp.result){
					$("#allCategory").modal("show");
				}else{
					msg = "종료되지 않은 광고가 있어<br>계속 진행할 수 없습니다.<br>문제가 지속될 경우 <a href='mailto:moca@innocean.com'>moca@innocean.com</a>로<br>연락 주시기 바랍니다.";	
	...
	let _data = {
		getData: function(){
			let url_v = "/member/agency/data/get";	
			comm.send(url_v, null, "POST", function(resp){
		...
		drawData: function(data){
			//기본정보
			{
				...
				//이메일
				{
					...
					_oldCompanyEmail = data.company_email;
			...
			//사업자 정보
			{
				...
				//사업장 주소
				$("#companyAddress1").val(util.unescapeData(data.zipcode+". "+data.address1));
				//상세주소
